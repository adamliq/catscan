...
Infoblox's 75 log categories, Zscaler's 28 log inputs, and Cisco
Catalyst SD-WAN's 47-row logging reference (see
...
and `cisco_sdwan_logging_reference.json` (from `other/data/`, see
below — a modest ~46&nbsp;KB, ~10&nbsp;KB, ~9&nbsp;KB, ~32&nbsp;KB,
~97&nbsp;KB, and ~21&nbsp;KB respectively, but all six fetched rather
...
`Juniper EX-series`, `DDI Infoblox`, `Zscaler`, and `Cisco SD-WAN` —
see below), each
...
A sixth vendor, Cisco Catalyst SD-WAN, brought the most raw material of
any vendor so far but the least single catalog shape — no confidence
rating, product split, or format axis runs through all of it the way
one axis runs through each of the other five vendors' own rows.
...
explanatory note, joining FortiManager's/Juniper's/Infoblox's as the
fourth vendor for whom that's the honest answer: the source's only
field-shaped data already lives in Reference, tied to a category, not
to individual Log Type rows, and the syslog messages' own `format`
strings are positional templates, not named fields.
...
- `other/` — data for the Other Events tab, one file per vendor, each
  ...
  provenance notes), and `data/cisco_sdwan_logging_reference.json` (a
...
FortiManager, Juniper EX-series, DDI Infoblox, Zscaler, and Cisco
SD-WAN each set inside `build_app_other()` (not a
